...
document.addEventListener("DOMContentLoaded", () => {
  ...
  const fmtDKK = new Intl.NumberFormat("da-DK",{style:"currency",currency:"DKK",maximumFractionDigits:0});
  const fmtPct = new Intl.NumberFormat("da-DK",{minimumFractionDigits:2,maximumFractionDigits:2});
  const toNum = (v)=>{ if(v==null) return 0; const x=parseFloat(String(v).replace(/\./g,'').replace(',', '.')); return Number.isFinite(x)?x:0; };
  const toPct = (v)=>{ if(v==null) return 0; const x=parseFloat(String(v).replace(',', '.')); return Number.isFinite(x)?x:0; };
  ...
  const PRESETS = { manual:null, global_index:0.20, danish_index:0.50, etf_broker:0.35, robo:0.75, single_stocks:0.00, crypto_hold:0.00 };
  let syncingFeesFromPreset = False = false; // ensure false
  ...
  function setFeesFromPreset(){
    ...
    if(p==null || !Number.isFinite(p)){ syncingFeesFromPreset=false; return; }
    try{ feesEl.valueAsNumber = p; }catch{}
  ...
  function monthlyIRR(cashflows, maxIter=100, tol=1e-7){
    function npv(r){ let v=0; for(let t=0;t<cashflows.length;t++) v += cashflows[t] / Math.pow(1+r,t); return v; }
    let lo=-0.999, hi=0.3, fLo=npv(lo), fHi=npv(hi);
  ...
  function computeProjection(initial, monthly, years, expPct, feePct){
    const rA = toPct(expPct)/100, fA = toPct(feePct)/100;
    const rM = monthlyFromAnnual(rA), fM = monthlyFromAnnual(fA);
    let bal = initial; const rows=[];
    ...
    return { rows, finalBalance: bal, totalContrib, totalGain: bal-totalContrib, effAnnual };
  }
  ...
  function drawChart(rowsB, band=null, rowsA=null){
    ...
    const padL=56, padR=10, padT=14, padB=40, innerW=W-padL-padR, innerH=H-padT-padB;
    ...
    ctx.strokeStyle="#94a3c9"; ctx.lineWidth=1;
    ...
    if(band && band.p10 && band.p90){
      ...
      ctx.closePath();
      ctx.fillStyle="rgba(255,165,0,.22)";
      ctx.fill();
    }
    if(band && band.p50){
      ctx.beginPath();
      ctx.strokeStyle="rgba(255,165,0,.95)"; ctx.lineWidth=2;
    ...
    ctx.beginPath();
    ctx.strokeStyle="rgba(108,168,255,1)"; ctx.lineWidth=2;
    ctx.moveTo(x(1), y(rowsB[0].balance));
    for(let i=1;i<rowsB.length;i++) ctx.lineTo(x(i+1), y(rowsB[i].balance));
    ctx.stroke();
    ctx.lineTo(x(rowsB.length), y(0)); ctx.lineTo(x(1), y(0)); ctx.closePath();
    ctx.fillStyle="rgba(108,168,255,.22)"; ctx.fill();

    if(rowsA && rowsA.length){
      ctx.beginPath();
      ctx.strokeStyle="rgba(99,230,190,1)"; ctx.lineWidth=2;
      ctx.moveTo(x(1), y(rowsA[0].balance));
      for(let i=1;i<rowsA.length;i++) ctx.lineTo(x(i+1), y(rowsA[i].balance));
      ctx.stroke();
      ctx.lineTo(x(rowsA.length), y(0)); ctx.lineTo(x(1), y(0)); ctx.closePath();
      ctx.fillStyle="rgba(99,230,190,.16)"; ctx.fill();
    }

    const initial = toNum(initialEl.value), monthly = toNum(monthlyEl.value);
    ctx.beginPath(); ctx.lineWidth=2; ctx.strokeStyle="#55607a";
    for(let i=0;i<rowsB.length;i++){
      const xx=x(i+1), contrib = initial + monthly*12*(i+1);
      const yy=y(contrib); if(i===0) ctx.moveTo(xx,yy); else ctx.lineTo(xx,yy);
    }
    ctx.stroke();

    ctx.fillStyle="#9fb0d1"; ctx.font="12px ui-sans-serif, system-ui, -apple-system, Segoe UI, Roboto, Arial";
  ...
  function renderTable(rows){
    const tbody = $("#year-table tbody"); if(!tbody) return;
    tbody.innerHTML = rows.map(r => (
      `<tr><td>${r.year}</td><td>${fmtDKK.format(r.contrib)}</td><td>${fmtDKK.format(Math.max(0,r.gain))}</td><td>${fmtDKK.format(r.balance)}</td></tr>`
    )).join("");
  }
  ...
  function clearUI(){
    ...
    const tbody = $("#year-table tbody"); if(tbody) tbody.innerHTML="";
    const tbody2 = $("#mc-table tbody"); if(tbody2) tbody2.innerHTML="";
    if(canvas){
  ...
  function runBaseCalc(){
    ...
    if(eff==null || !isFinite(eff)){
      const rA = toPct(expEl.value)/100, fA = toPct(feesEl.value)/100;
      eff = (1+rA)/(1+fA) - 1;
    }
    if(eff==null || !isFinite(eff) || isNaN(eff) || eff<-0.5){
      kCagr.textContent="—"; chartCagr.textContent="—";
    }else{
      const s = fmtPct.format(eff*100)+" %";
      kCagr.textContent=s; chartCagr.textContent=s;
    }
  ...
  function initTips(){
    document.querySelectorAll(".tip").forEach(el=>{
      const btn = el.querySelector(".tip-btn"); const tip = el.querySelector(".tooltip");
      if(!btn||!tip) return;
      btn.addEventListener("click", (e)=>{
        e.stopPropagation();
        document.querySelectorAll(".tip.open").forEach(o=>{ if(o!==el) o.classList.remove("open"); });
  ...
  btnPNG?.addEventListener("click", ()=>{
    ...
    const a = document.createElement("a"); a.href=dataURL; a.download="finlytics-chart.png"; a.click();
  });
  ...
  btnCSVYr?.addEventListener("click", ()=>{
    ...
    download("finlytics-year.csv", toCSV(rows, ["År","Samlet indskud","Afkast","Saldo"]), "text/csv");
  });

  btnCSVMc?.addEventListener("click", ()=>{
    if(!mcBody||!mcBody.children.length) return;
    const rows = Array.from(mcBody.querySelectorAll("tr")).map(tr => Array.from(tr.children).map(td => td.textContent));
    download("finlytics-montecarlo.csv", toCSV(rows, ["År","P10","P50","P90"]), "text/csv");
  });

  btnShare?.addEventListener("click", async function(){
    const params = new URLSearchParams({
      initial:String(initialEl?.value||""), monthly:String(monthlyEl?.value||""), years:String(yearsEl?.value||""),
      exp:String(expEl?.value||""), fees:String(feesEl?.value||""), feePreset:String(feePresetEl?.value||""),
      vol:String(volEl?.value||""), sims:String(simsEl?.value||"")
    });
  ...
  function makePDFReport(){
    try{
      ...
      w.document.write(`
        <html><head><meta charset="utf-8" /><title>Finlytics – Rapport</title>
        <style>
          ...
          .kpi b{display:block;font-size:12px;color:#555;margin-bottom:4px}
          .kpi div{font-size:16px;font-weight:700}
          .inputs table{width:100%;border-collapse:collapse}
          .inputs th,.inputs td{border-bottom:1px solid #eee;padding:6px 4px;text-align:left;font-size:14px}
          .chart{margin-top:10px;border:1px solid #eee;border-radius:8px;padding:8px;text-align:center}
        ...
        <div class="grid">
          <div class="card inputs">
            <h2>Inputs</h2>
            <table>
              <tr><th>Startbeløb</th><td>${initialEl.value||""}</td></tr>
              <tr><th>Månedligt indskud</th><td>${monthlyEl.value||""}</td></tr>
              <tr><th>Antal år</th><td>${yearsEl.value||""}</td></tr>
              <tr><th>Forventet årligt afkast</th><td>${expEl.value||""} %</td></tr>
              <tr><th>Årlige omkostninger</th><td>${feesEl.value||""} % (preset: ${feePresetEl.value||"manual"})</td></tr>
            </table>
          </div>
          <div class="card">
            <h2>KPI’er</h2>
            <div class="kpis">
              ...
              <div class="kpi"><b>Årligt effektivt afkast</b><div>${kpiCagrTxt}</div></div>
            </div>
          </div>
        </div>
        <div class="card chart">
          <h2>Graf</h2>
          ${chartImg ? `<img src="${chartImg}" style="max-width:100%;height:auto" />` : "<em>Ingen graf</em>"}
        </div>
        <div class="foot">Genereret ${now}</div>
        <script>window.addEventListener('load',()=>setTimeout(()=>window.print(), 250));</script>
        </body></html>
      `);
      w.document.close();
    }catch(e){ console.error(e); alert("Kunne ikke generere PDF-rapport."); }
  }

  function handleClearAll(){
    try{
      ["#initial","#monthly","#years","#expReturn","#fees","#volatility","#simulations"]
        .forEach(sel => { const el=$(sel); if(el) el.value=""; });
      if(feePresetEl) feePresetEl.value="manual";
      try{ localStorage.removeItem("scenarioA"); }catch{}
  ...
  function simulateMC(years, rA, fA, initial, monthly, sims, sigPct){
    const muM = monthlyFromAnnual(rA), feeM = monthlyFromAnnual(fA), sigM = (sigPct/100)/Math.sqrt(12);
    const p10=[], p50=[], p90=[];
    for(let y=1;y<=years;y++){
      const vals=[];
  ...
  function loadA(){ try{ const raw=localStorage.getItem("scenarioA"); return raw?JSON.parse(raw):null; }catch{return null} }
  function saveA(obj){ try{ localStorage.setItem("scenarioA", JSON.stringify(obj)); }catch{} }

  function cagrStr(res, expPct, feePct){
    let eff = res.effAnnual;
    if(eff==null || !isFinite(eff)){ const rA=toPct(expPct)/100, fA=toPct(feePct)/100; eff=(1+rA)/(1+fA)-1; }
    if(eff==null || !isFinite(eff) || isNaN(eff) || eff<-0.5) return "—";
  ...
  function handleSaveA(){
    ...
    saveA(snap);
    if(btnSaveA_big){ const t=btnSaveA_big.textContent; btnSaveA_big.textContent="A gemt ✓"; setTimeout(()=>btnSaveA_big.textContent=t, 1000); }
    if(cmpBox){ cmpBox.hidden=false; cmpAFinal.textContent="—"; cmpACagr.textContent="—"; cmpBFinal.textContent="—"; cmpBCagr.textContent="—"; }
    if(cmpKPIsBox) cmpKPIsBox.hidden=true;
  }
  ...
  function handleCompare(){
    const A = loadA();
    if(!A){
      [btnCompare_small, btnCompare_big].forEach(b=>{ if(!b) return; const t=b.textContent; b.textContent="Gem A først"; setTimeout(()=>b.textContent=t, 1000); });
      return;
    }
    const initB=toNum(initialEl.value), monB=toNum(monthlyEl.value), yrsB=Math.max(1,parseInt(yearsEl.value||"1",10));
    const expB=toPct(expEl.value), feeB=toPct(feesEl.value);
    const initA=toNum(A.initial), monA=toNum(A.monthly), yrsA=Math.max(1,parseInt(A.years||"1",10));
    const expA=toPct(A.exp), feeA=toPct(A.fees);
    ...
    if(cmpKPIsBox){
      const gainA=Math.max(0,resA.totalGain), gainB=Math.max(0,resB.totalGain);
      ...
      const sA=cagrStr(resA,expA,feeA), sB=cagrStr(resB,expB,feeB);
      const pp=(sA==="—"||sB==="—") ? "—" : fmtPct.format(parseFloat(sB.replace(',','.')) - parseFloat(sA.replace(',','.'))) + " pp";
      cmpAContrib.textContent = fmtDKK.format(resA.totalContrib);
      cmpBContrib.textContent = fmtDKK.format(resB.totalContrib);
      cmpDContrib.textContent = (diffContrib>=0?"+":"") + fmtDKK.format(Math.abs(diffContrib)).replace("kr.","kr.");
      cmpAGain.textContent = fmtDKK.format(gainA);
      cmpBGain.textContent = fmtDKK.format(gainB);
      cmpDGain.textContent = (diffGain>=0?"+":"") + fmtDKK.format(Math.abs(diffGain)).replace("kr.","kr.");
      cmpAFinal2.textContent = fmtDKK.format(resA.finalBalance);
      cmpBFinal2.textContent = fmtDKK.format(resB.finalBalance);
      cmpDFinal.textContent  = (diffFinal>=0?"+":"") + fmtDKK.format(Math.abs(diffFinal)).replace("kr.","kr.");
  ...
  function handleClearCompare(){
    currentBand=null; runBaseCalc(); if(cmpBox) cmpBox.hidden=true; if(cmpKPIsBox) cmpKPIsBox.hidden=true;
  }

  $("#calc-form")?.addEventListener("submit", (e)=>{ e.preventDefault(); currentBand=null; runBaseCalc(); });
  document.addEventListener("keydown", (e)=>{ if(e.key==="Enter" && document.activeElement && document.activeElement.tagName==="INPUT"){ e.preventDefault(); currentBand=null; runBaseCalc(); } });
  initTips();
  ...
  btnReport?.addEventListener("click", (e)=>{ e.preventDefault(); makePDFReport(); });
  btnClearAll?.addEventListener("click", (e)=>{ e.preventDefault(); handleClearAll(); });

  const touchManual = ()=>{ if(feePresetEl && feePresetEl.value!=="manual") feePresetEl.value="manual"; };
  ...
  mcBtn?.addEventListener("click", ()=>{
    ...
    const rA=toPct(expEl.value)/100, fA=toPct(feesEl.value)/100;
    const initial=toNum(initialEl.value), monthly=toNum(monthlyEl.value);
    const sims=Math.min(3000, Math.max(50, parseInt(simsEl.value||"1000",10)));
    const vol=Math.max(0, toPct(volEl.value));
    const band=simulateMC(years, rA, fA, initial, monthly, sims, vol);
    currentBand=band;
    const base = computeProjection(initial, monthly, years, toPct(expEl.value), toPct(feesEl.value));
    drawChart(base.rows, currentBand, null);
    if(mcBody){
      mcBody.innerHTML = base.rows.map((r,i)=>`<tr><td>${i+1}</td><td>${fmtDKK.format(band.p10[i])}</td><td>${fmtDKK.format(band.p50[i])}</td><td>${fmtDKK.format(band.p90[i])}</td></tr>`).join("");
    }
  });
